...
collapedEnsemblTranscriptIds = ensemblOverlapping$overlappers[which(ensemblOverlapping$overlappers$collapsedGene == ""),]$collapsed ### some ensembl gene ids do not have gene names, i want to retain this info for later as well...
...
intron_total = lapply(intron_divided_PAS,function(x) do.call(rbind.data.frame,x))
intron_total = do.call(rbind.data.frame,intron_total)  
intron_total$peakClassification = row.names(intron_total)
write.table(intron_total,paste0(InPath, "/intron_total.bed"),sep="\t",quote = F,row.names = F,col.names = F)

exon_total = lapply(exon_divided_PAS,function(x) do.call(rbind.data.frame,x))
exon_total = do.call(rbind.data.frame,exon_total)  
exon_total$peakClassification = row.names(exon_total)
write.table(intron_total,paste0(InPath, "/exon_total.bed"),sep="\t",quote = F,row.names = F,col.names = F)
...
refSeq_divided_PAS_signalSum = 
  lapply(refSeq_divided_PAS,function(x) lapply(x,function(y) sum(as.numeric(unlist(strsplit(y$V5,",",T))))))

ensembl_divided_PAS_signalSum = 
  lapply(ensembl_divided_PAS,function(x) lapply(x,function(y) sum(as.numeric(unlist(strsplit(y$V5,",",T))))))

exons_divided_PAS_signalSum = 
  lapply(exon_divided_PAS,function(x) lapply(x,function(y) sum(as.numeric(unlist(strsplit(y$V5,",",T))))))

introns_divided_PAS_signalSum = 
  lapply(intron_divided_PAS,function(x) lapply(x,function(y) sum(as.numeric(unlist(strsplit(y$V5,",",T))))))

nonOverlapping_divided_PAS_signalSum = 
  lapply(nonOverlapping_divided_PAS,function(x) lapply(x,function(y) sum(as.numeric(unlist(strsplit(y$V5,",",T))))))

refSeq_divided_PAS_signalSum = melt(refSeq_divided_PAS_signalSum)
ensembl_divided_PAS_signalSum = melt(ensembl_divided_PAS_signalSum)
exons_divided_PAS_signalSum = melt(exons_divided_PAS_signalSum)
introns_divided_PAS_signalSum = melt(introns_divided_PAS_signalSum)
nonOverlapping_divided_PAS_signalSum = melt(nonOverlapping_divided_PAS_signalSum)


common = nonOverlapping_divided_PAS_signalSum[,c(2,3)]

refSeqOverlapping_signal = refSeq_divided_PAS_signalSum$value
ensemblOverlapping_signal = ensembl_divided_PAS_signalSum$value
exonOverlapping_signal = exons_divided_PAS_signalSum$value
intronOverlapping_signal = introns_divided_PAS_signalSum$value
nonOverlapping_signal = nonOverlapping_divided_PAS_signalSum$value
allSignal = cbind.data.frame(common,refSeqOverlapping_signal,ensemblOverlapping_signal,exonOverlapping_signal,intronOverlapping_signal,nonOverlapping_signal)
write.table(allSignal,paste0(InPath,"/signalSummary.txt"),sep="\t",quote = F)
