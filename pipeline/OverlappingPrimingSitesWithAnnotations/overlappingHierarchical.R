...
checkPAS = function(query_threshold)
{
  ...
  query_AATAAA = dplyr::filter(query_threshold,grepl("AATAAA",upstreamSequences))
  query_threshold = dplyr::filter(query_threshold,!grepl("AATAAA",upstreamSequences))
  query_ATTAAA = dplyr::filter(query_threshold,grepl("ATTAAA",upstreamSequences))
  query_threshold = dplyr::filter(query_threshold,!grepl("ATTAAA",upstreamSequences))
  query_APA = dplyr::filter( query_threshold,grepl(paste(toMatch,collapse="|"),upstreamSequences))
  query_minusAPA= dplyr::filter( query_threshold,!grepl(paste(toMatch,collapse="|"),upstreamSequences))
  queryStats = list(query_AATAAA,query_ATTAAA, query_APA, query_minusAPA)
  names(queryStats) = c("AATAAA","ATTAAA","APA","noPAS")
  return(queryStats)
