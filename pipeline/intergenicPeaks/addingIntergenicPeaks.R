#BOut="//clustertmp/bioinfo/pooja/SLAMannotation/dr_allData//output/"
...
counts_inUTR100_counts_subset = apply(counts_inUTR100_counts, 1,function(x) length(which(x>10)))


#### but for the zebrafish annotation, I am using RNAseq daatsets from different stages, so If I take a mean of 10 reads across all stages, i would miss out on some stage specific signals... 
#### to avoid this, i want to extend, if even 1 rnaSeq sample shows reads >10 reads. 
...
#customAnnotation = customAnnotation[which(counts_inUTR100_counts_subset>=1),]
